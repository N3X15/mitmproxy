...
class Writer(_FileLike):
    ...
    def flush(self):
        """
            May raise exceptions.UdpDisconnect
        """
        if hasattr(self.o, "flush"):
            try:
                self.o.flush()
            except OSError as v:
                raise exceptions.UdpDisconnect(str(v))
    ...
    def write(self, v):
        ...
        if v:
            ...
            try:
                ...
                raise exceptions.UdpDisconnect(str(e))
...
class Reader(_FileLike):
    ...
    def safe_read(self, length):
        ...
        if length != -1 and len(result) != length:
            if not result:
                raise exceptions.UdpDisconnect()
            else:
                raise exceptions.UdpReadIncomplete(
    ...
    def peek(self, length):
        ...
        Raises:
            exceptions.UdpException if there was an error with the socket
        ...
        if isinstance(self.o, socket_fileobject):
            try:
                ...
                raise exceptions.UdpException(repr(e))
        else:
            raise NotImplementedError("Can only peek into (pyOpenSSL) sockets")


def read_select(rlist, timeout):
    """
    ...
    def finish(self):
        ...
        if not getattr(self.wfile, "closed", False):
            try:
                self.wfile.flush()
                self.wfile.close()
            except exceptions.UdpDisconnect:
                pass
...
class UDPClient(_Connection):
    ...
    def create_connection(self, timeout=None):
        ...
        err = None
        for res in socket.getaddrinfo(self.address[0], self.address[1], 0, socket.SOCK_DGRAM):
